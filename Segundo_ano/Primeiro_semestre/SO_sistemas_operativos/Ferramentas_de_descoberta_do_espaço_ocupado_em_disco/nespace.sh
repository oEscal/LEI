...
#$4 é -d
#$5 é -L
#$e é uma string onde estão os ficheiros a serem ignorados
   ...
   sum=0
	...
	if [[ $NA -ne 0 && $(ls $path) != "" ]]; then
		...
		if [[ $NA -ne 0 && $(ls -Spd "$path/"* | grep -v /$) != "" ]]; then	#-S é para ordenar por tamanho e -p é para adicionar / no final das diretorias
      	...
      	array_files=()
			for i in ${files[@]}; do	#tratamento da opção -n e -e
				v=1
				for n in $e; do
					if [[ "$i" == "$n" ]]; then
						v=0
      	   ...
      	   if [[ $v -eq 1 ]] && [[ $( echo $i | rev | cut -d "/" -f1 | rev ) =~ $2 ]]; then
